...
**Institutional Account Linking V1 [LIVE] (2026-09-09)**: migration 86
...
[LIVE] (2026-09-10)**: migration 87 makes
...
the existing module flags preserved. Release image `8b89068` (digest
`sha256:dbf553b7d4a09ac9b9f250c65956c624fbca441857dc23fb6b527e88ceff8b49`)
was verified as candidate and 5% canary, then promoted to 100% on Cloud Run
revision `privydoc-doc-workspace-tenantadmin-8b89068`; the prior healthy
revision remains tagged as the rollback target.
...
session restoration and stale local projections can no longer infer an
unlinked identity; a genuinely authenticated unlinked identity retains the
...
path: `src/modules/communication/components/{CommunicationHubView,CapabilityDelegationPanel,CapabilityBadge}.tsx`, `src/modules/communication/lib/{communicationDomain,communicationService}.ts`; routes `/workspace/communication` and `/doctor/communication`; schema `supabase/migrations/{84_communication_hub_v1,85_secure_delivery_gateway_v1}.sql`; server boundary `supabase/functions/workspc-gateway/` with pure contracts in `supabase/functions/_shared/workspcGatewayCore.ts`
